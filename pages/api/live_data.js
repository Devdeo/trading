...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Now make the actual API request (FIXED: removed double equals)
    ...
    console.error('NSE Live Data API error:', error.message);
    
    // Return cached data if available during errors
    if (cache.data && cache.timestamp && now - cache.timestamp < 300000) { // 5 minute fallback
      console.log('Returning cached live data due to API error');
      return res.status(200).json(cache.data);
    }
    ...
    if (error.response?.status === 401 || error.response?.status === 403) {
      ...
      res.status(500).json({ 
        error: 'Failed to fetch live data from NSE API',
